...
function Plane(scene, nrDivs, xRatio = 1, yRatio = 1) {
...
Plane.prototype.initBuffers = function() {
	...
	*/
	...
	var yCoord = 0.5;
	var yTextCoord = 0.5 - ((this.patchLength/this.yRatio) * (this.nrDivs/2));
	console.log(yTextCoord);

	for (var j = 0; j <= this.nrDivs; j++) 
	{
		var xCoord = -0.5;
		var xTextCoord = 0.5 - ((this.patchLength/this.xRatio) * (this.nrDivs/2));
		for (var i = 0; i <= this.nrDivs; i++) 
		{
			...
			this.texCoords.push(xTextCoord,yTextCoord);

			xTextCoord += this.patchLength/this.xRatio;
			xCoord += this.patchLength;
		}
		yCoord -= this.patchLength;
		yTextCoord += this.patchLength/this.yRatio;
	}
